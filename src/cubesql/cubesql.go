...
func (this *CubeSQL ) Select( sql string ) *CubeSQLCursor {
  cSQL := C.CString( sql )
  defer C.free( unsafe.Pointer( cSQL ) )

  cursor := CubeSQLCursor { cursor: C.cubesql_select( this.db, cSQL, C.int( FALSE ) ) }
  return &cursor
}
...
func (this *CubeSQL ) Bind( sql string, values []interface{} ) int {
  ...
  for index, interfaceValue := range values {
    ...
    case int:
      v := fmt.Sprintf( "%d", int( value ) )
      coltype[ index ] = BIND_INTEGER
      colsize[ index ] = C.int( 1 + len( v ) )
      colval[ index ]  = C.CString( v )
...
func( this *CubeSQLCursor ) GetField( row int, column int ) string {
  var len C.int = 0
